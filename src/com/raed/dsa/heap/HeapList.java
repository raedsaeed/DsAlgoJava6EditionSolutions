...
public class HeapList<T extends Comparable<T>> implements Heap<T> {
    ...
    public HeapList(Comparator<T> comparator) {
        this(null, comparator);
    }

    public HeapList(T[] array, Comparator<T> comparator) {
        this.comparator = comparator;
        heapify(array);
    }

    @SuppressWarnings("unchecked")
    public HeapList(Collection<T> list) {
        T[] array = (T[]) new Object[list.size()];
        heapify(list.toArray(array));
    }
    ...
    @Override
    public void upHeap(int index) throws IllegalArgumentException {
        checkIndex(index);
        int child = index;
    ...
    @Override
    public void downHeap(int index) throws IllegalArgumentException {
        checkIndex(index);
        int parent = index;
        while (parent < size() - 1) {
